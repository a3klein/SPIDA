...
def generate_random_box_bounds(
    frame_width: int, frame_height: int, box_width: int, box_height: int, **kwargs
) -> tuple[int, int, int, int]:
    """
    ...
    """
    x, y, w, h = generate_random_box(
        frame_width, frame_height, box_width, box_height, **kwargs
    )
    return (x, y, x + w, y + h)


def generate_random_box_center(
    frame_width: int, frame_height: int, box_width: int, box_height: int, **kwargs
) -> tuple[int, int, int, int]:
    """
...
def generate_multiple_random_boxes(
    frame_width: int,
    frame_height: int,
    box_width: int,
    box_height: int,
    ...
    **kwargs,
    ...
    if not avoid_overlap:
        ...
        if len(boxes) < num_boxes:
            print(
                f"Warning: Could only place {len(boxes)} out of {num_boxes} non-overlapping boxes"
            )
