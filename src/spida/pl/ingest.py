...
def plot_shapes(
    ...
    sdata.pl.render_shapes(
        SHAPES_KEY, table_name=table_name, method="matplotlib"
    ).pl.show(ax=axes[0], title="all shapes", coordinate_systems=cs)
    sdata.pl.render_shapes(
        SHAPES_KEY,
        ...
        cmap="viridis",
    ).pl.show(ax=axes[1], title="Transcript Count", coordinate_systems=cs)
    sdata.pl.render_shapes(
        SHAPES_KEY,
        table_name=table_name,
        scale=2,
        ...
        color="volume",
        cmap="viridis",
    ).pl.show(ax=axes[2], title="Volume", coordinate_systems=cs)
...
def plot_overlap(
    ...
    sdata.pl.render_images(IMAGE_KEY, scale=image_scale_keys[-1]).pl.render_shapes(
        SHAPES_KEY
    ).pl.show(ax=axes[0], title="Shapes on Image", coordinate_systems=cs)
    sdata.pl.render_images(IMAGE_KEY, scale=image_scale_keys[-1]).pl.render_points(
        POINTS_KEY, groups=gene_ex, color="gene", cmap="tab10"
    ).pl.show(ax=axes[1], title="Points on Image", coordinate_systems=cs)
    sdata.pl.render_shapes(SHAPES_KEY).pl.render_points(
        POINTS_KEY, groups=gene_ex, color="gene", cmap="tab10"
    ).pl.show(ax=axes[2], title="Points on Shapes", coordinate_systems=cs)
...
def plot_seg_load(sdata, image_key, shapes_key, cs="global", pdf_file=None):
    ...
    ).pl.show(ax=axes[0], title="DAPI", coordinate_systems=cs, colorbar=False)
    visualize_boxes(
