...
def tile_image_with_overlap(image : np.ndarray,
    ...
    if len(tile_size) != image.ndim:
        raise ValueError(f"tile_size dimensions ({len(tile_size)}) must match image dimensions ({image.ndim})")
    
    if len(overlap) != image.ndim:
        raise ValueError(f"overlap dimensions ({len(overlap)}) must match image dimensions ({image.ndim})")
...
def save_tiles(tiles, tile_info, output_dir, prefix="tile", format="tif", func : Callable[[np.ndarray], np.ndarray] =None):
    """
...
def save_projected_tiles_from_files(input_dir, tile_info, output_dir=None, prefix="tile", input_format="tif", output_format="tif", input_suffix=".decon", output_suffix=".decon.2d", projection_method="max", z_slice_range=None):
    ...
    for info in tile_info:
        ...
        if len(pos) == 2:  # 2D
            ...
            base_filename = f"{prefix}_{tile_id:04d}_d{pos[0]}_r{pos[1]}_c{pos[2]}_col{col}"
...
def merge_tile_polygons(tile_info_list, polygon_gdfs, coordinate_offset=(0, 0), geom_col="geometry"):
    """
    ...
    for tile_info, gdf in zip(tile_info_list, polygon_gdfs):
        ...
        if len(tile_position) == 2:
            ...
            raise ValueError(f"Unsupported tile position dimensions: {len(tile_position)}")
        ...
        if len(coordinate_offset) == 2:
            ...
            raise ValueError(f"Unsupported coordinate_offset dimensions: {len(coordinate_offset)}")
        ...
        for idx, row in gdf.iterrows():
            ...
            if hasattr(polygon, 'exterior'):  # Single polygon
                ...
                transformed_coords = [(x + total_col_offset, y + total_row_offset) 
                ...
                if polygon.interiors:
                    ...
                    for interior in polygon.interiors:
                        ...
                        transformed_hole = [(x + total_col_offset, y + total_row_offset) 
                                          for x, y in hole_coords]
                        holes.append(transformed_hole)
                    transformed_polygon = shp.geometry.Polygon(transformed_coords, holes)
                    
            else:
                # Handle other geometry types (MultiPolygon, etc.)
                transformed_polygon = shp.affinity.translate(polygon, 
                                                           xoff=total_col_offset, 
                                                           yoff=total_row_offset)
    ...
    if merged_polygons:
        ...
        return gpd.GeoDataFrame(columns=[geom_col, 'tile_id', 'original_tile_position', 
                                       'tile_row_offset', 'tile_col_offset']).set_geometry(geom_col)
