...
def read_info(path: str | Path) -> pd.DataFrame: 
    ...
    df_info = df_data_org[['channelName', 'imageType', 'imagingRound', 'color', 'frame', 'zPos']]
    df_info = df_info.rename(columns = {'channelName': 'channel', 'imagingRound' : "round", 'imageType': "prefix"})
    # assign types
    df_info = df_info.astype({"channel" : str, 
                            "prefix" : str,
                            "round" : str, 
                            "color" : int, 
                            "frame" : 'object', 
    ...
    # formate frame
    df_info['frame'] = df_info['frame'].apply(lambda x: np.asarray(x.strip("[]").split(","), dtype=int))
    df_info['zPos'] = df_info['zPos'].apply(lambda x: np.asarray(x.strip("[]").split(","), dtype=float))
    # get file pattern 
    df_info['file_pattern'] = df_info['prefix'] + df_info['round'] + "_{fov:04d}.jp2.zip"
...
def read_zip(
    ...
    for _f in frames: 
        ...
        cv_img = cv2.imdecode(np.frombuffer(zip_img.read(), dtype=dtype),
...
def read_img(
    ...
    attrs["stage_position"] = np.asarray(attrs['position'].strip("()").split(","), dtype=np.float32)
