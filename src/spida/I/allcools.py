...
def _allcools_pca(ref_adata:ad.AnnData,
...
def _transfer_labels(adata, integrator, rna_cell_type='supercluster_term'): 
    ...
    adata.obs[f'allcools_{rna_cell_type}'] = adata.obs.index.to_series().map(label_transfer[rna_cell_type].apply(lambda x: labels[np.argmax(x)], axis=1).to_dict())
    adata.obs[f'allcools_{rna_cell_type}_transfer_score'] = adata.obs.index.to_series().map(label_transfer[rna_cell_type].apply(lambda x: x[np.argmax(x)], axis=1).to_dict())
    adata.obs['all_annot'] = [adata.obs.loc[x, rna_cell_type] if pd.isnull(adata.obs.loc[x, f'allcools_{rna_cell_type}']) else adata.obs.loc[x, f'allcools_{rna_cell_type}'] for x in adata.obs.index]
    return adata


def run_allcools_seurat(ref_adata:ad.AnnData,
                        qry_adata:ad.AnnData, 
    ...
    **kwargs: Additional keyword arguments for SeuratIntegration.
    ...
    ref_adata, qry_adata = _allcools_pca(ref_adata, qry_adata, n_train_cell=n_train_cell, chunk_size=chunk_size)
