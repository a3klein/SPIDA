...
def write_adata_register_subparser(subparser):
    ...
    parser.add_argument(
        "--reg_name",
        type=str,
        help="Name of the region (optional, if not provided writes all regions)",
    )
    parser.add_argument(
        "--prefix_names", type=parse_list, required=True, help="List of prefix names"
    )
    parser.add_argument(
        "--output_path",
        type=parse_path,
...
def setup_adata_region_register_subparser(subparser):
    ...
    parser.add_argument(
        ...
        type=str,
        default="_filt",
        help="Suffix for the output table key in the spatialdata object (default: '_filt')",
    )
    parser.add_argument(
        "--plot", action="store_true", help="Whether to plot the results"
    )
...
def resolvi_region_register_subparser(subparser):
    ...
    parser.add_argument(
        ...
        type=str,
        default="_filt",
        help="Suffix for the table key in the spatialdata object (default: '_filt')",
    )
    parser.add_argument(
        "--plot", action="store_true", help="Whether to plot the results"
    )
...
def remove_doublets_region_register_subparser(subparser):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Threshold for doublet detection (default: 0.5)",
    )
    parser.add_argument(
        "--suffix",
        type=str,
        default="",
        help="Suffix for the keys in the spatialdata object (default: '')",
    )
    parser.add_argument(
        "--plot", action="store_true", help="Whether to plot the results"
    )
...
def plot_setup_region_register_subparser(subparser):
    ...
    parser.add_argument("--image_path", type=parse_path, help="Path to save the plot")
    parser.add_argument(
        "--suffix", type=str, default="_filt", help="Suffix for the table key (default: '_filt')"
    )
    return

def plot_resolvi_region_register_subparser(subparser):
    """Register subparser for plot-resolvi-region command."""
    parser = subparser.add_parser(
        "plot-resolvi-region",
        aliases=["plot_resolvi_region"],
        help="Plot the resolvi results for a specific region",
        description="Plot the resolvi results for a specific region in an experiment",
    )
    parser.add_argument("exp_name", type=str, help="Name of the experiment")
    parser.add_argument("reg_name", type=str, help="Name of the region")
    parser.add_argument(
    ...
    parser.add_argument("--image_path", type=parse_path, help="Path to save the plot")
    parser.add_argument(
        "--suffix", type=str, default="_filt", help="Suffix for the table key in the spatialdata object (default: '_filt')"
    )
    return
...
def combine_datasets_register_subparser(subparser):
    ...
    parser = subparser.add_parser(
        ...
        description="Combine multiple datasets into a single SpatialData object + AnnData object",
    )
    parser.add_argument(
        "experiment_list", type=parse_list, help="list of experiment names"
    )
    parser.add_argument(
        "prefix_name", type=str, help="Prefix for the keys in the spatialdata object"
    )
    ...
    parser.add_argument(
        "--lab_name", type=str, default="salk", help="Name of the lab (default: 'salk')"
    )
    parser.add_argument(
        "--project_name",
        type=str,
        default="BG_salk",
        help="Name of the project (default: 'BG_salk')",
    )
    parser.add_argument(
        "--zarr_path", type=parse_path, help="Path to the Zarr directory"
    )
    parser.add_argument(
        "--output_path", type=parse_path, help="Path to the output directory"
    )
    parser.add_argument(
        "--table_keys", type=parse_list, help="List of table keys to include"
    )
    return

def setup_dataset_register_subparser(subparser):
    """Register subparser for setup_dataset command."""
    parser = subparser.add_parser(
        "setup-dataset",
        aliases=["setup_dataset"],
        help="Perform standard PCA + Leiden clustering on a joint dataset object",
        description="Perform standard PCA + Leiden clustering on a joint dataset object",
    )
    parser.add_argument("dataset_name", type=str, help="name of the dataset")
    parser.add_argument("--scale", type=bool, default=False, help="Whether to scale the data (default: False)")
    parser.add_argument("--anndata_path", type=parse_path, help="Path to the AnnData directory")
    return

def resolvi_dataset_register_subparser(subparser):
    """Register subparser for resolvi_dataset command."""
    parser = subparser.add_parser(
        "resolvi-dataset",
        aliases=["resolvi_dataset"],
        help="Perform ResolVI workflow on a joint dataset object",
        description="Perform ResolVI workflow on a joint dataset object",
    )
    parser.add_argument("dataset_name", type=str, help="name of the dataset")
    parser.add_argument("--anndata_path", type=parse_path, help="Path to the AnnData directory")
    parser.add_argument("--model_save_path", type=parse_path, default=None, help="Path to save the ResolVI model")
...
def main():
    ...
    elif command in ["plot-resolvi-region"]:
        from .main import plot_resolvi_region as func
    elif command in ["combine-datasets"]:
        from .main import combine_datasets as func
    elif command in ["setup-dataset"]:
