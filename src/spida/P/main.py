...
def filter_cells_region(exp_name:str,
    ...
    logger.info("DONE")
    if plot:
        plot_filtering_region(exp_name, reg_name, prefix_name, image_path=image_path, suffix="_filt")
...
def setup_adata_region(exp_name:str, 
    ...
    KEYS = _gen_keys(prefix_name, exp_name, reg_name)
    # Get the AnnData object
    adata = _get_adata(exp_name, reg_name, prefix_name, suffix="_filt")  # Use the filtered data if available
    # Run the setup
    ...
    if plot:
        plot_setup_region(exp_name, reg_name, prefix_name, image_path=image_path, suffix="_filt")
